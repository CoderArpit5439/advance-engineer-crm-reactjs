  ...
  (error) => {
    if (error.message === "network error") {
      if (error?.response?.status === 504) {
        toast.error(
          "Gateway Timeout (504): Something went wrong. Please try again later."
        );
      } else {
        toast.error(
          "Network Error: You are not connected. Please verify your connection."
        );
      }
    }
    if (error.response) {
      ...
      switch (status) {
        ...
        case 500:
          toast.error(
            "Internal Server Error (500): Something went wrong. Please try again later."
          );
          break;
        default:
